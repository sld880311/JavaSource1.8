...
//    计算ctl新值（线程状态与线程个数）
        ...
        implements Runnable
    {
        ...
        // worker所对应的第一个任务
        // 表示该worker所包含的Runnable对象，即用户自定义的Runnable对象，完成用户自定义的逻辑的Runnable对象
        Runnable firstTask;
     ...
     * 5.当worker执行完一个任务的时候，包括初始任务firstTask，会调用getTask()继续获取任务，这个方法调用
     */
     ...
     * @param maximumPoolSize the maximum number of threads to allow in the
     *        pool
     *        -- 线程池允许创建的最大线程数。如果队列满了，并且已创建的线程数小于最大线程数，
...
//            如果当前线程池为空，则添加一个线程
     ...
     * -用于被拒绝任务的处理程序，它放弃最旧的未处理请求，然后重试 execute；
     * -如果执行程序已关闭，则会丢弃该任务
     */
