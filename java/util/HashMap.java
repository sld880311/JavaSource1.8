...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * 用于扩容的加载因子(使用0.75可以提供空间利用率，
     * 降低hash冲突，使链表或者红黑树的高低比较低提升空间效率)
     */
    ...
    final void putMapEntries(Map<? extends K, ? extends V> m, boolean evict) {
        int s = m.size();
        if (s > 0) {
            if (table == null) { // pre-size，如果table为空则需要进行初始化
            	/**
                 * 计算需要的容量：实际容量 = 容量*0.75，
                 * 反向计算并且+1（小数向上取整）
                 * 
                 */
    ...
    final Node<K,V>[] resize() {
        ...
        /**
         * 如果oldCap<0，但是已经初始化了，像把元素删除完之后的情况，
         * 那么它的临界值肯定还存在，        
         * 如果是首次初始化，它的临界值则为0
         */
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                Node<K,V> e;
                /**
                 * 当前hash桶的位置不为空，表示数组下表下存在值，由于有值则表示可能存在冲突
                 */
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null; // 赋值为null，方便释放历史数据内存
                    if (e.next == null) // 处理当前下标节点没有元素的情况
                        newTab[e.hash & (newCap - 1)] = e;
                    // 该节点为红黑树结构，也就是存在哈希冲突，该哈希桶中有多个元素
                    else if (e instanceof TreeNode)
                    	// 把红黑树的老数据转换到newTab中
                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);
                    else { // preserve order，转换链表数据到newTab中
